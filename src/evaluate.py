...
def evaluate_prompt(
    ...
    try:
        ...
        max_examples = int(os.getenv("MAX_EVAL_EXAMPLES", "0"))
        if max_examples > 0:
        ...
        # Counter para progress display
        completed = {"n": 0}
        total = len(examples)
        f1_scores, clarity_scores, precision_scores = [], [], []

        print("   Avaliando exemplos e publicando como Experiment no LangSmith...")

        # Target: gera a User Story
        def target(inputs: dict) -> dict:
            response = chain.invoke(inputs)
            return {"answer": response.content}

        # Avaliador combinado (sequencial) — também coleta scores localmente
        def combined_evaluator(run, example):
            ...
            completed["n"] += 1
            print(f"      [{completed['n']}/{total}] F1:{f1['score']:.2f} Clarity:{clarity['score']:.2f} Precision:{precision['score']:.2f}")
        ...
        # Passa examples (já limitado por MAX_EVAL_EXAMPLES) ao invés do dataset_name
        # para que ls_evaluate respeite o limite local
        data_arg = examples if max_examples > 0 else dataset_name
        ...
        ls_evaluate(
            ...
            description=f"Avaliação automática de {prompt_name}",
            metadata={"prompt_name": prompt_name, "provider": provider, "model": model},
            max_concurrency=1,
            client=client,
        )
